...
def create_key_pair():
    if not os.path.exists(key_path):
        ...
        with os.fdopen(
            os.open(key_path, os.O_WRONLY | os.O_CREAT, 0o400), "w+"
        ) as handle:
...
def create_instance():
    ...
    if "ec2_instance_ids" in ec2_data:
        ...
        print(ec2_data["ec2_instance_ids"].append(instance_id))
    else:
...
# get_public_ip("i-020bd7d52a7310f6f")
...
# reboot_instance("i-020bd7d52a7310f6f")
...
# stop_instance("i-020bd7d52a7310f6f")
...
# start_instance("i-020bd7d52a7310f6f")
...
# terminate_instance("i-020bd7d52a7310f6f")

# terminate multiple ec2 instances
def terminate_instances(instance_ids):
    response = ec2_client.terminate_instances(InstanceIds=instance_ids)
    print(response)
    final_instances_list = list(
        filter(lambda item: (item not in instance_ids), ec2_data["ec2_instance_ids"])
    )
