...
# upload_file("./sample_file_2.txt", bucket_name, "sample_file_2.txt")
...
# delete_empty_bucket("eampty-bucket-example-sandip")
...
#     "./downloaded_files/sample_file_2.txt", bucket_name, "sample_file_2.txt"
# )
...
def download_file_concurrently(file_name, bucket, object_name):
    ...
    try:
        response = s3_client.download_file(
            bucket, object_name, file_name, Config=config
        )
    except ClientError as e:
        logging.error(e)
        return False
...
#         uploadOject["url"], data=uploadOject["fields"], files=files
#     )
...
def change_object_permission(bucket, object_name, permission):
    ...
    try:
        response = s3_client.put_object_acl(
            Bucket=bucket, Key=object_name, ACL=permission
        )
    except ClientError as e:
        logging.error(e)
        return None
